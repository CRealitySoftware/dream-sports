...
export default function UsersPage() {
  ...
  async function handleSendWelcome(user: UserRow, toEmail: string) {
    await sendWelcomePayment({ name: user.name, email: user.email, discipline: user.discipline }, toEmail);
  }
  ...
  async function handleSendFreeWelcome() {
    ...
    setSendingFree(true);
    await sendWelcomePayment({}, freeEmail.trim());
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          >
            <Ionicons name="mail-outline" size={16} color="rgba(8,61,145,1)" />
          </Pressable>
          ...
          <View style={{
            ...
            {freeSent ? (
              <View style={{ alignItems: "center", gap: 12, paddingVertical: 12 }}>
                ...
                <Text style={{ color: colors.ink, fontSize: 15, fontWeight: "700" }}>Correo enviado</Text>
                <Text style={{ color: colors.inkMuted, fontSize: 13 }}>{freeEmail}</Text>
              </View>
            ) : (
              <>
                <View style={{ gap: 2 }}>
                  <Text style={{ color: colors.ink, fontSize: 15, fontWeight: "800" }}>Enviar bienvenida</Text>
                  <Text style={{ color: colors.inkMuted, fontSize: 12 }}>Confirmación de pago manual</Text>
                </View>
